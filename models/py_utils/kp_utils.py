...
def _decode(
    ...
    br_scores = br_scores.view(batch, 1, K, 1, 1).expand(batch, K, K, K, K)
    tr_scores = tr_scores.view(batch, 1, 1, K, 1).expand(batch, K, K, K, K)
    bl_scores = bl_scores.view(batch, 1, 1, 1, K).expand(batch, K, K, K, K)
    scores    = (tl_scores + br_scores + tr_scores + bl_scores) / 4
    ...
    width_inds1  = (br_xs < tl_xs)
    width_inds2  = (bl_xs > tr_xs)
    height_inds1 = (br_ys < tl_ys)
    height_inds2 = (bl_ys > tr_ys)

    # scores[cls_inds]    = -1
    scores[dist_inds]   = -1
    scores[width_inds1]  = -1
    scores[height_inds1] = -1
    scores[width_inds2]  = -1
    scores[height_inds2] = -1
